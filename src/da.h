...
#define da_append_many(da, items, items_size) do { \
        if ((da)->size + (items_size) > (da)->capacity) {\
            if ((da)->capacity == 0) {\
                (da)->capacity = DA_INIT_CAP; \
            } \
            while ((da)->size + (items_size) > (da)->capacity) {\
                (da)->capacity = (da)->capacity * 2; \
            } \
            (da)->items = realloc((da)->items, (da)->capacity * sizeof((da)->items[0])); \
            assert((da)->items != NULL); \
        } \
        memcpy((da)->items + (da)->size, items, (items_size) * sizeof(*((da)->items))); \
        (da)->size += items_size; \
    } while (0)
...
#define da_last(da) (da)->items[(da)->size - 1]
